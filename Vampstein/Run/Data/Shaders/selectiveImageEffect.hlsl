...
float4 FragmentFunction( VertexToFragment_t input ) : SV_Target0 // semeantic of what I'm returning
{
    ...
    {
        float gradient = 0.5 + 0.5 * sin( 2.0 * SYSTEM_TIME - 33.0 * uvToUse.x + 33.0 * uvToUse.y );
        gradient = 1.0 - 0.5 * ( gradient * gradient * gradient * gradient * gradient * gradient * gradient * gradient );
        color.r *= gradient;
        color.g *= gradient;
    }

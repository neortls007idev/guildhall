...
- [x] Allow player to move the camera by change the camera transform position
   - [x] `Q` & `E`: Forward & Back (movement is relative to where you're looking)
   - [x] `A` & `D`: Left and Right (movement is relative to where you're looking)
   - [x] `W` & `S`: Up and Down Movement (movement is absolute (world up and world down)
...
- [x] Generate a **UV Sphere** mesh during `Game::Startup`
    - [x] `vertexUtils.hpp` has a function
...
- [x] *X04.11: 02%*: **Requires X04.10** - Mouse options are disabled when window loses focus, and re-applied when gaining focus.( not doing using that InputSYstemFunction).
- [x] *X02.11 : 04pts*:  Directory Watch.  Write a system that prints when a file changes (is added, removed, or changed) in your data folder during run-time. *( still using FileFirstChangeNotification, can be found in rendercontext ).*
- [x] *x02.12 : 02pts*:  **Requires X02.10 and X02.11**:  Auto reload shaders when changed on disc. *( wait for object has been set to 3 milliseconds which is like 20% the frame rate already).*

...
void Game::KeyboardKeyPressEvents()
{
	...
	if ( g_theInput->WasKeyJustPressed('N') == true && g_theApp->m_isPaused == false)
	{
		if (m_Ship->m_isDead)
		{
			delete m_Ship;
			m_Ship = nullptr;
			m_Ship = new PlayerShip(g_theApp->m_theGame, Vec2(WORLD_CENTER_X, WORLD_CENTER_Y), Vec2(0.f, 0.f), 0.f);
		}
	}
}
...
void Game::Render() const
{
	...
	for (int BulletPointerArrayIndex = 0; BulletPointerArrayIndex < MAX_BULLETS; BulletPointerArrayIndex++)
	{
 	...
 	{
 		if (m_Asteroids[AsteroidPointerArrayIndex] != nullptr && m_Asteroids[AsteroidPointerArrayIndex]->m_isGarbage == false )
 		{
...
void Game::RenderUI() const
{
	...
	for ( --lifeIcons; lifeIcons > 0 ; lifeIcons-- )
	{
		...
		TransformVertexArray2D(NUM_SHIP_VERTS, lives, 10.f, 0.f, Vec2((10.f + (50*lifeIcons)), UI_SIZE_Y - 50.f));
...
void Game::UpdateBettles(float deltaSeconds)
{
	for (int BettlePointerArrayIndex = 0; BettlePointerArrayIndex < MAX_BETTLES; BettlePointerArrayIndex++)
	{
		if (m_Bettles[BettlePointerArrayIndex] != nullptr && m_Bettles[BettlePointerArrayIndex]->m_isGarbage == false)
		{
...
void Game::SpawnDebrisField(const Vec2& center, int howmany, const Rgba8& color)
{
	...
	for (int index = 0; index < howmany; index++)
	{
		...
		if (!m_Debris[DebrisPointerArrayIndex])
		{
			...
			m_Debris[DebrisPointerArrayIndex] = new Debris(this, temp + Vec2(x, y), Vec2(5.f, 5.f), orientationOffset, color);
			break;
		}
		}
	}
...
{
	g_theRenderer->DrawLine(m_Asteroids[AsteroidPointerArrayIndex]->m_position, (m_Asteroids[AsteroidPointerArrayIndex]->m_position + m_Asteroids[AsteroidPointerArrayIndex]->m_velocity), Rgba8(255, 255, 0, 255), 0.25f);
	g_theRenderer->DrawLine(m_Ship->m_position, m_Asteroids[AsteroidPointerArrayIndex]->m_position, Rgba8(50, 50, 50, 255), 0.25f);
	g_theRenderer->DrawRing(m_Asteroids[AsteroidPointerArrayIndex]->m_position, m_Asteroids[AsteroidPointerArrayIndex]->m_cosmeticRadius, m_Asteroids[AsteroidPointerArrayIndex]->m_cosmeticRing, 0.25f);
	g_theRenderer->DrawRing(m_Asteroids[AsteroidPointerArrayIndex]->m_position, m_Asteroids[AsteroidPointerArrayIndex]->m_physicsRadius, m_Asteroids[AsteroidPointerArrayIndex]->m_physicsRing, 0.25f);
}

void Game::BulletDebugDrawing(int BulletPointerArrayIndex) const
{
	g_theRenderer->DrawLine(m_Ship->m_position, m_Bullets[BulletPointerArrayIndex]->m_position, Rgba8(50, 50, 50, 255), 0.25f);
	g_theRenderer->DrawRing(m_Bullets[BulletPointerArrayIndex]->m_position, m_Bullets[BulletPointerArrayIndex]->m_cosmeticRadius, m_Bullets[BulletPointerArrayIndex]->m_cosmeticRing, 0.25f);
	g_theRenderer->DrawRing(m_Bullets[BulletPointerArrayIndex]->m_position, m_Bullets[BulletPointerArrayIndex]->m_physicsRadius, m_Bullets[BulletPointerArrayIndex]->m_physicsRing, 0.25f);
	g_theRenderer->DrawLine(m_Bullets[BulletPointerArrayIndex]->m_position, (m_Bullets[BulletPointerArrayIndex]->m_velocity + m_Bullets[BulletPointerArrayIndex]->m_position), Rgba8(255, 255, 0, 255), 0.25f);
}

void Game::ShipDebugDrawing() const
{
	g_theRenderer->DrawLine(m_Ship->m_position, (m_Ship->m_velocity + m_Ship->m_position), Rgba8(255, 255, 0, 255), 0.25f);
	...
	for (int bulletIndex = 0; bulletIndex < MAX_BULLETS; bulletIndex++)
	{
		if (m_Bullets[bulletIndex] != nullptr && m_Bullets[bulletIndex]->m_isGarbage != true)
		{
			for (int astroidIndex = 0; astroidIndex < MAX_ASTEROIDS; astroidIndex++)
			{
				...
				{
					if (DoDiscsOverlap(m_Bullets[bulletIndex]->m_position, m_Bullets[bulletIndex]->m_physicsRadius, m_Asteroids[astroidIndex]->m_position, m_Asteroids[astroidIndex]->m_physicsRadius))
					{
						AddScreenShakeIntensity(0.3f);
						m_Asteroids[astroidIndex]->m_health--;
						m_Bullets[bulletIndex]->m_isGarbage = true;
						SpawnDebrisField(m_Bullets[bulletIndex]->m_position, NUM_BULLET_VERTS / 3, DEBRISBULLETCOLOR);
						SpawnDebrisField(m_Asteroids[astroidIndex]->m_position, NUM_ASTEROID_VERTS / 3, DEBRISASTEROIDCOLOR);
	...
	for (int bulletIndex = 0; bulletIndex < MAX_BULLETS; bulletIndex++)
	{
		if (m_Bullets[bulletIndex] != nullptr && m_Bullets[bulletIndex]->m_isGarbage != true)
		{
			for (int bettleIndex = 0; bettleIndex < MAX_BETTLES; bettleIndex++)
			{
				...
				{
					if (DoDiscsOverlap(m_Bullets[bulletIndex]->m_position, m_Bullets[bulletIndex]->m_physicsRadius, m_Bettles[bettleIndex]->m_position, m_Bettles[bettleIndex]->m_physicsRadius))
					{
						m_Bettles[bettleIndex]->m_health--;
						AddScreenShakeIntensity(0.4f);
						m_Bullets[bulletIndex]->m_isGarbage = true;
						SpawnDebrisField(m_Bettles[bettleIndex]->m_position, NUM_BETTLE_VERTS / 3, DEBRISBETTLECOLOR);
						SpawnDebrisField(m_Bullets[bulletIndex]->m_position, NUM_BULLET_VERTS / 3, DEBRISBULLETCOLOR);
...
void Game::DetectCollissionBetweenAstroidAndShip()
{
	for (int index = 0; index < MAX_ASTEROIDS; index++)
	{
		if (m_Asteroids[index] != nullptr)
		{
			if (DoDiscsOverlap(m_Ship->m_position, m_Ship->m_physicsRadius, m_Asteroids[index]->m_position, m_Asteroids[index]->m_physicsRadius))
			{
				m_Ship->m_health--;
...
void Game::DetectCollissionBetweenBettleAndShip()
{
	for (int index = 0; index < MAX_BETTLES; index++)
	{
		if (m_Bettles[index] != nullptr)
		{
			if (DoDiscsOverlap(m_Ship->m_position, m_Ship->m_physicsRadius, m_Bettles[index]->m_position, m_Bettles[index]->m_physicsRadius))
			{
				m_Ship->m_health--;
				if (m_Ship->m_health > 0)
				{
...
void Game::DetectCollissionBetweenWaspAndShip()
{
	for (int index = 0; index < MAX_WASPS; index++)
	{
		if (m_Wasps[index] != nullptr)
		{
			if (DoDiscsOverlap(m_Ship->m_position, m_Ship->m_physicsRadius, m_Wasps[index]->m_position, m_Wasps[index]->m_physicsRadius))
			{
				m_Ship->m_health--;
				if (m_Ship->m_health > 0)
				{

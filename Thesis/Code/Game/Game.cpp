...
//	m_objCityMeshTex_Normal[ ORGANODRON_CITY ]		= g_theRenderer->GetOrCreateTextureFromFile( "Data/Models/Organodron City/Maps/b1b2.jpg" );
	...
	//m_objScifiShipMeshTex_Diffuse[ DELOREAN ]		= g_theRenderer->GetOrCreateTextureFromFile( "Data/Models/Luminaris_OBJ/Luminaris Diffuse.png" );
	//m_objScifiShipMeshTex_Normal[  DELOREAN ]		= g_theRenderer->GetOrCreateTextureFromFile( "Data/Models/Luminaris_OBJ/Luminaris Normal.png" );
...
Game::~Game()
{
	...
	{
		delete m_objCityMesh[ index ];
		m_objCityMesh[ index ]			= nullptr;
	}
	
	for ( int index = 0 ; index < NUM_SCIFI_SHIPS ; index++  )
	{
		delete m_objSciFiShipMeshes[ index ];
		m_objSciFiShipMeshes[ index ]			= nullptr;
	}
...
void Game::InitializeCameras()
{
		...
		m_gameCamera.SetProjectionPerspective( 60.f , CLIENT_ASPECT , -.00001f , -10000.f );
		m_gameCamera.SetPosition( Vec3( 0.f , 0.f , 200.f ) );
...
void Game::Update( float deltaSeconds )
{
	...
	DebugAddScreenTextf( Vec4( 0.f , 0.f , 0.625f , 0.9815f ) , Vec2::ONE , 20.f , PURPLE , deltaSeconds * 0.5f ,
...
//	g_theRenderer->DrawMesh( m_objCityMesh[ ORGANODRON_CITY ] );
	...
	g_theRenderer->SetModelMatrix( m_objSciFiShipTransform[ LUMINARIS ].GetAsMatrix() );
 	g_theRenderer->DrawMesh( m_objSciFiShipMeshes[ LUMINARIS ] );
...
{	
	...
	m_gameCamera.SetPitchYawRollRotation( m_pitch , finalRoll , m_yaw );
}

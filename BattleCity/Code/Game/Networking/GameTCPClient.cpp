...
GameTCPClient::GameTCPClient( SOCKET socket , Mode mode /*= Mode::Nonblocking */ ) : m_mode( mode )
{
...
SOCKET GameTCPClient::Connect( std::string const& host , std::uint16_t port , Mode mode /*= Mode::Nonblocking */ )
{
	m_mode = mode;
...
std::string GameTCPClient::ReceiveServerMessage( SOCKET server , char* bufferAddr , int bufferLength )
{
...
void GameTCPClient::SendServerMessage( SOCKET server )
{

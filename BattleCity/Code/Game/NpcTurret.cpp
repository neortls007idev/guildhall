...
void NpcTurret::Update( float deltaSeconds )
{
	...
	if ( m_theGame->m_world->m_currentMap->m_entityListsByType[ PLAYERTANK_ENTITY ][ 0 ] )
	{
		playerPosition = m_theGame->m_world->m_currentMap->m_entityListsByType[ PLAYERTANK_ENTITY ][ 0 ]->m_position;
	}
	...
	if ( m_theGame->m_world->m_currentMap->m_entityListsByType[ PLAYERTANK_ENTITY ][ 0 ] )
	{
		if ( m_theGame->m_world->m_currentMap->InLineOfSight( playerPosition , m_position , m_lineOfSightRadius ) && m_theGame->m_world->m_currentMap->m_entityListsByType[ PLAYERTANK_ENTITY ][ 0 ]->m_health > 0 )
		{
			...
			{
				m_laser = m_theGame->m_world->m_currentMap->RayCast( m_position , ForwardDirection , displacementBetweenTurretAndPlayer.GetLength() );
			}
			else
			{
				m_laser = m_theGame->m_world->m_currentMap->RayCast( m_position , ForwardDirection , m_lineOfSightRadius );
			}
		...
		else if ( m_lastSeen )
		{
			...
			m_laser = m_theGame->m_world->m_currentMap->RayCast( laserPosition , Vec2( CosDegrees( m_turretOrientation ) , SinDegrees( m_turretOrientation ) ) , m_lineOfSightRadius );
		}
		else
		{
			...
			m_laser = m_theGame->m_world->m_currentMap->RayCast( laserPosition , Vec2::MakeFromPolarDegrees( m_turretOrientation ) , m_lineOfSightRadius );
		}

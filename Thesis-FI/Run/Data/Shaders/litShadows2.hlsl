...
fragmentFunctionOutput FragmentFunction( v2f_t input )
{
   ...
   // {
        float2 mapDim;
        depthMapTexture0.GetDimensions( mapDim.x , mapDim.y );
   ...
   // Determine if the projected coordinates are in the 0 to 1 range.  If so then this pixel is in the view of the light.
        ...
        float result2 = depthValue <= s2;
        float result3 = depthValue <= s3;

        float2 texelPos = deltaPixel * projectTexCoord[ index ].xy;
